defmodule Explorer.Etherscan do
  ...
  alias Explorer.Chain.{Address, Block, Hash, InternalTransaction, TokenTransfer, Token.Instance,Transaction}
  ...
  *NOTE*
  ...
  {:ok, main_address_hash} = Chain.string_to_address_hash("0xfe0e612a60e8a4477138faffde468488df42ef1e")
  @main_address_hash main_address_hash
  @doc """
  ...
  def list_tokens(%Hash{byte_count: unquote(Hash.Address.byte_count())} = address_hash) do
    query =
      from(
        ...
        on:
        ctb.token_id == instance.token_id,
        where: ctb.address_hash == ^address_hash,
#        美幻地址的余额不进行查询
#        where: ctb.address_hash != ^@main_address_hash,
        where: ctb.value > 0,
        where: not is_nil(ctb.token_type),
        select: %{
          balance: ctb.value,
