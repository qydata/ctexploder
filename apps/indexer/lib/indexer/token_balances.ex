defmodule Indexer.TokenBalances do
  ...
  * `token_id` - token id for ERC-1155 tokens
  ...
  def fetch_token_balances_from_blockchain(token_balances) do
    ...
    #    Logger.debug(fn -> ["fetch_token_balances_from_blockchain_iozhaq: ", inspect(token_balances)] end)
    regular_token_balances =
      token_balances
      |> Enum.filter(
           fn request ->
             if Map.has_key?(request, :token_type) do
               request.token_type !== "ERC-1155" && request.token_type !== "ERC-721"
             else
               true
             end
           end
    ...
    erc721_token_balances =
      token_balances
      |> Enum.filter(
           fn request ->
             if Map.has_key?(request, :token_type) do
               request.token_type == "ERC-721"
             else
               false
             end
           end
    ...
    #      这里需要查询token Id ,这样只需要判断tokenId就可以来进行设置
    requested_erc721_token_balances =
      erc721_token_balances
      |> BalanceReader.get_balances_of_with_abi(@erc721_balance_function_abi)
      |> Stream.zip(erc721_token_balances)
      |> Enum.map(fn {result, token_balance} -> set_token_balance_value721(result, token_balance) end)
    ...
    requested_token_balances = requested_regular_token_balances ++ requested_erc721_token_balances ++ requested_erc1155_token_balances
  ...
  def to_address_current_token_balances(address_token_balances) when is_list(address_token_balances) do
    ...
    |> Enum.map(
         fn {_, grouped_address_token_balances} ->
  ...
  defp set_token_balance_value721({:ok, balance}, token_balance) do
    if token_balance.address_hash == balance do
      Map.merge(token_balance, %{value: 1, value_fetched_at: DateTime.utc_now(), error: nil})
    else
      Map.merge(token_balance, %{value: 0, value_fetched_at: DateTime.utc_now(), error: nil})
    end

  end

  defp set_token_balance_value721({:error, error_message}, token_balance) do
    #    if "(3) execution reverted: ERC721: owner query for nonexistent token" <> _ = error_message do
    if String.starts_with?(error_message, "(3) execution reverted: ERC721: owner query for nonexistent token") do
      Map.merge(token_balance, %{value: 0, value_fetched_at: DateTime.utc_now(), error: nil})
    else
      #    (3) execution reverted: ERC721Psi: owner query for nonexistent token
      if String.starts_with?(error_message, "(3) execution reverted: ERC721Psi: owner query for nonexistent token") do
        Map.merge(token_balance, %{value: 0, value_fetched_at: DateTime.utc_now(), error: nil})
      else
        Map.merge(token_balance, %{value: nil, value_fetched_at: nil, error: error_message})
      end
