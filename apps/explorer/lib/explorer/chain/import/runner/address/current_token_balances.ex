  ...
  def insert_changes_list_with_and_without_token_id(changes_list, repo, timestamps, timeout, options) do
    ...
    %{
      ...
      |> Enum.reduce(%{changes_list_no_token_id: [], changes_list_with_token_id: []}, fn change, acc ->
        updated_change =
          if Map.has_key?(change, :token_id) and ( Map.get(change, :token_type) == "ERC-721" or  Map.get(change, :token_type) == "ERC-1155") do
#            if Map.get(change, :token_type) == "ERC-721" do
#              if Map.get(change, :value) > 1 do
#                Map.put(change, :value, 1)
#              else
#                Map.put(change, :value, 0)
#              end
#            else
              change
#            end
